...
export default function AdminDashboardPage() {
  ...
  const [counts, setCounts] = useState({
    students: 0, coaches: 0, courses: 0, tournaments: 0, events: 0, news: 0
  });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    (async () => {
      const [s, c, cr, t, e, n] = await Promise.all([
      ...
      setCounts({
        students: s.count || 0, coaches: c.count || 0, courses: cr.count || 0,
        tournaments: t.count || 0, events: e.count || 0, news: n.count || 0
      });
      setLoading(false);
    })();
  }, []);
  ...
  const stats = [
    { label: "Total Students", value: counts.students, icon: Users, href: "/admin/students", color: "text-blue-600 bg-blue-50" },
